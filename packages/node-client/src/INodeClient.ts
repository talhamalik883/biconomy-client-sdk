...
interface INodeClient {
  ...
  // 8. Cross Chain Endpoints

  /**
   * About: Estimate the gas cost (in gasTokenAddress) for the cross-chain transaction originating from the specified rawSourceChainTx
   * @return CrossChainGasEstimateResponse object containing the gas cost of the cross-chain transaction
   */
  getCrossChainGasEstimate(
    crossChainGasEstimate: CrossChainGasEstimateDto
  ): Promise<CrossChainGasEstimateResponse>

  // 9. Liquidity Pool Endpoints
  /**
   * About: Given a set of transfer parameters, run checks like sufficient balance, sufficient allowance, sufficient liquidity, etc.
   * @param preDepositCheckDto
   */
  preDepositCheck(preDepositCheckDto: PreDepositCheckDto): Promise<PreDepositCheckResponse>

  /**
   * About: Returns a list of supported tokens on chain
   * @param chainId
   */
  getCrossChainSupportedTokens(chainId: number): Promise<string[]>

  /**
   * About: Returns the details of the liquidity pool specified by the DTO
   * @param getLiquidityPoolInfoDto
   */
  getLiquidityPoolInfo(getLiquidityPoolInfoDto: GetPoolInfoDto): Promise<GetPoolInfoResponse>

  /**
   *
   */
  getTokenTransferFeeEstimate(
